...
#define _TARGET_HIGH 360.0  // 27cm
...
void loop() {
  float dist_raw;
  int servoPosition; // Add servo position variable
  ...
  // Adjust servo position according to distance
  if (dist_raw <= _TARGET_LOW) {
    // Distance is less than or equal to 18cm, set servo to 0 degrees
    servoPosition = _DUTY_MIN;
  } else if (dist_raw >= _TARGET_HIGH) {
    // Distance is greater than or equal to 36cm, set servo to 180 degrees
    servoPosition = _DUTY_MAX;
  } else {
    // Map distance between 18cm and 36cm to servo angle between 0 and 180 degrees
    servoPosition = map(dist_raw, _TARGET_LOW, _TARGET_HIGH, _DUTY_MIN, _DUTY_MAX);
  }
